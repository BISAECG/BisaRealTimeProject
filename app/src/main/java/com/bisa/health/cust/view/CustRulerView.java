...
public class CustRulerView extends View {
    ...
    private float mTextSize =25;         //尺子刻度下方数字 textsize
    ...
    public CustRulerView(Context context) {
        this(context, null);

    }

    public CustRulerView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CustRulerView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void setValue(float selectorValue, float minValue, float maxValue, float per) {
        ...
        Log.i("----","mOffset--           "+mOffset  +"         =====mMaxOffset    "+mMaxOffset
    ...
    private void countVelocityTracker() {
        Log.i("zkk---","countVelocityTracker-");
    ...
    private void countMoveEnd() {
        ...
        int BitNumber=5-((int)mSelectorValue)%10;
     
        
        if(BitNumber==1){
        	mSelectorValue=Math.round(mSelectorValue/10f)*10f+5f;
        }else if(BitNumber==-1){
        	   mSelectorValue=Math.round(mSelectorValue/10f)*10f-5f;
        }else if(BitNumber==0){
        	 mSelectorValue=mSelectorValue/10f*10f;
        }else{
        	   mSelectorValue=Math.round(mSelectorValue/10f)*10f;
        }
